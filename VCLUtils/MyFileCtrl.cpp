...
{
    const AnsiString
        exeFileName = ExtractFileName(fnExe),
        fn1 = ExtractFileName(fn),
        cmd = params.IsEmpty() ?
            MYSPRINTF_("%s %s", exeFileName, fn1 ) :
            MYSPRINTF_("%s %s%s", exeFileName, params, fn1 ),
    ...
    ::CreateProcess
    (
        fnExe.c_str(),	            // pointer to name of executable module
        cmd.c_str(),	        // pointer to command line string
...
{
    std::ifstream srcFile(fn.c_str(), std::ios::binary | std::ios::in);
    std::size_t sz = GetStreamSize(srcFile);
    dst.resize(sz+1, '\0');
...
{
    std::ifstream srcFile(fn.c_str(), std::ios::binary | std::ios::in);
    const std::size_t sz = GetStreamSize(srcFile);
    dst.resize(sz);
    srcFile.read( &dst.at(0), sz );
    srcFile.close();
}
...
void PutStringToFile(const AnsiString& fn, const char* pText, unsigned len)
{
    ...
    std::ofstream outputFile( fn.c_str(), std::ios::binary );
    outputFile.write( pText, len );
    outputFile.close();
}
//------------------------------------------------------------------------------
void PutStringToFile(const AnsiString& fn, const std::vector<char>& dst)
{
    PutStringToFile( fn, &dst.at(0), dst.size() );
}
//------------------------------------------------------------------------------
