...
struct DefaultFoo
{
	void AfterSetCellCanvas( TStringGrid*,int,int,MyGridCellState, TRect, int&, int&, bool&)    {}
	void AfterDraw(TStringGrid*, int,int, MyGridCellState, TRect)    {}
    bool BeforDrawLine(TStringGrid*,int,int,MyGridCellState,unsigned, TRect)    {return true;}
    void OnCellBoundChange( TStringGrid*, int, int)    {}
    void OnDeterminateColumnWidth( TStringGrid*, int, int, bool&)    {}
    void OnGetCellText( TStringGrid*, int, int, AnsiString& ) {}
};
...
void DrawWrapCellText( TCanvas& cnv, const TRect& rect,
    AnsiString Sentence, bool draw, int* pLineCount = NULL, int *pCurY = 0)
{
    int varLineCount, varCurY,
        &lineCount = pLineCount==NULL ? varLineCount : *pLineCount,
        &CurY = pCurY==NULL ? varCurY : *pCurY;
    lineCount = 1;
...
public:
    ...
    DrawCellFoo::OnGetCellText onGetCellText_;
...
DrawGridCellManager::Impl::Impl(TStringGrid* grd, bool mustRegisterOnMouseOccurrence) :
        ...
        onGetCellText_(&(defaultFoo_.DefaultFoo::OnGetCellText)),
        hotCell_(-1,-1),
...
const TRect& Rect, TGridDrawState State)
{
    ...
    const Grd::GridCell cell = Grd::GridCellT(grd_,ACol,ARow);
    const bool
    ...
    onGetCellText_(grd_, ACol, ARow, cellText );
    ...
    if (!my::Grd::IsRowSelected(grd_,ARow))   {
        cnv.Brush->Color   = my::Grd::unselectedRowColor;
    }

    afterSetCanvas_(grd_, ACol, ARow, myGridState, textRect, txtX,  txtY, wrapText );
    ...
    if(!wrapText)
        ...
        cellText = CELLS[ACol][ARow];
        int lineCount, curY;
        DrawWrapCellText( cnv, Rect, cellText, false, &lineCount, &curY );
        const int allHeight = std::min( curY - Rect.Top + grd_->DefaultRowHeight, 100);
        if( allHeight>grd_->RowHeights[ARow] )
            grd_->RowHeights[ARow] = allHeight;
        else {
            TRect wrapRect = Rect;
            wrapRect.Top = wrapRect.Top + (wrapRect.Height() - lineCount*textH)/2-textH/4 ;
            wrapRect.Top = std::max(Rect.Top, wrapRect.Top);
            DrawWrapCellText( cnv, wrapRect, cellText, true );
        }
    }
    // обрамление
...
void DrawGridCellManager::Impl::PerformCustomDrawData(int ACol, int ARow)
{
    ...
    } else
    {
        ...
        } else
        {
            ...
            cnv.Brush->Color = Grd::IsCellFixed(Grd::GridCellT(grd_,ACol,ARow)) ? grd_->FixedColor : grd_->Color;
        }
    }
}
...
void DrawGridCellManager::SetOnOnGetCellText(DrawCellFoo::OnGetCellText foo)
{
    impl_->onGetCellText_ = foo;
}
...

{
    ...
    if ( Grd::IsCellFixed( Grd::GridCellT(grd,col,row))  )
