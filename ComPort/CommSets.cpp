...
std::vector<AnsiString> SystemPorts()
{
    ...
    err = GetLastError();  //err = 1722 at this point
    for(int n=0; n<returned; ++n){
        if (boost::regex_match(port_into_1s[n].pName,  xResults, xRegEx)) {
            const AnsiString portName = xResults[1].str().c_str();
